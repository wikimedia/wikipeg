...
function generateJavascript(ast, options) {
  ...
  function savePosRefState(node, result) {
    ...
    for (let name in rule.passedParams) {
      if (rule.passedParams[name].type === 'reference') {
        saved.refs.push(name);
      }
    }
    result.block.push(`${saved.dst} = ${saved.src};`);
    for(let name of saved.refs) {
      result.block.push(language.cacheSaveRef(name));
    }
    return saved;
  }

  function restorePosRefState(saved) {
    let block = [];
    block.push(`${saved.src} = ${saved.dst};`);
    for(let name of saved.refs) {
      block.push(language.cacheRestoreRef(name));
    }
    return block;
  }

  function freePosRefState(saved, result) {
    freeReg(saved.dst, result);
  }
  ...
  function buildSimplePredicate(node, context) {
    ...
    var saved = savePosRefState(node, result);
    ...
    if (negate) {
      result.onFailure(restorePosRefState(saved));
    } else {
      result.onSuccess(restorePosRefState(saved));
    }
    freePosRefState(saved, result);
    return result;
  }
  ...
  function buildRepeatedCharacterClass(node, context, atLeastOne) {
    var result = new Result();
    var reg = context.getResultReg(result);
    language.matchRepeatedClass(node.expression, reg, result, atLeastOne, context.getDiscard());
    result.onFailure([`${reg} = ${language.failed};`]);
   ...
   * comparison in getCacheStoreRefs() or restoration.
   */
