...
function generateJavascript(ast, options) {
  ...
  function addConst(obj) {
    ...
    if (str in constIndexes) {
      return language.constExpression(constIndexes[str]);
    } else {
      var index = consts.length;
      constIndexes[str] = index;
      consts.push(language.constDeclaration(index, str));
      return language.constExpression(index);
    }
  }
  ...
  function makeFailCall(value, context) {
    ...
    var call = language.libraryCall('fail', [constValue]);
    if (silence === 'false') {
      return call + ';';
    } else {
  ...
  var generate = visitor.build({
    ...
    zero_or_more: function(node, context) {
      ...
      result.block.push(`while (${partReg} !== ${language.failed}) {`);
      if (!context.getDiscard()) {
        result.block.push(indent2(language.push(resultReg, partReg)));
      }
      result.block.push(
        indent2(subresult.resolveBlock().join('\n')),
        '}');
    ...
    one_or_more: function(node, context) {
      ...
      var subresult = recurse(node.expression, newContext);
      result.append(subresult);
      result.onFailure([`${resultReg} = ${language.failed};`]);
      ...
      successBlock.push(`while (${partReg} !== ${language.failed}) {`);
      if (!context.getDiscard()) {
        successBlock.push(`  ${language.push(resultReg, partReg)}`);
      }
      successBlock.push(
        indent2(subresult.resolveBlock().join('\n')),
        '}');
  ...
  templateVars['/*END_CONST*/'] = addConst({ type: "end", description: "end of input" });
  ...
  templateVars['/*CONSTS*/'] = consts.join('\n');
  templateVars['/*ACTIONS*/'] = actions.join('\n');

  if (!ast.initializer || ast.initializer.length === 0) {
    templateVars['/*INITIALIZER0*/'] = '';
    templateVars['/*INITIALIZER*/'] = '';
  } else if (ast.initializer.code) {
    templateVars['/*INITIALIZER0*/'] = '';
    templateVars['/*INITIALIZER*/'] = ast.initializer.code;
  } else if (ast.initializer.length === 1) {
    templateVars['/*INITIALIZER0*/'] = '';
    templateVars['/*INITIALIZER*/'] = ast.initializer[0].code;
  } else {
    templateVars['/*INITIALIZER0*/'] = ast.initializer[0].code;
    templateVars['/*INITIALIZER*/'] = ast.initializer[1].code;
  }
