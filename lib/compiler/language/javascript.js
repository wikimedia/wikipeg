...
let javascript = {
  ...
  matchRepeatedClass(node, reg, result, atLeastOne, discarded) {
    let regexp;
    if (node.parts.length === 0) {
      if (node.inverted) {
        // Same as .* / .+
    ...
    if (discarded) {
      ...
      result.onSuccess([
        `peg$currPos += ${reg}[0].length;`,
        `${reg} = Array.from(${reg}[0]);`
      ]);
    }
  },
  ...
  cacheRestoreRef(name) {
    return `param_${name}.value = saved_${name};`;
  },
