...
let javascript = {
  ...
  constExpression(index) {
    return `peg$c${index}`;
  },

  constDeclaration(index, expression) {
  ...
  cacheLoadRef(name) {
    return `    if (cached.refs.${name} !== undefined) param_${name}.value = cached.refs.${name};`;
  },

  cacheStoreRef(name) {
    return `if (saved_${name} !== param_${name}.value) cached.refs.${name} = param_${name}.value;`;
  },
